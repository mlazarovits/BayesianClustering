...
class KUCMSTimeCalibration : public KUCMS_RootHelperBaseClass {
...
inline void KUCMSTimeCalibration::makeCaliHists(){
 ...
 inline kucms_DetIDStruct& KUCMSTimeCalibration::getDetIdInfo( uInt rhid ){
 ...
 inline std::map<int,std::map<int,std::map<int,uInt>>> KUCMSTimeCalibration::getInvDetIDMap(){

	return InvDetIDMap;

}//<<>>DetIDStruct KUCMSTimeCalibration::getInvDetIDMap( )

 inline std::map<UInt_t,kucms_DetIDStruct> KUCMSTimeCalibration::getDetIDMap(){; // map of information by detid for each crystal
...
//**************  main calibrate &/or smearing function based on tag + info ********************************************************************
 ...
 inline float KUCMSTimeCalibration::getTimeResoltuion( float amplitude, unsigned int rechitID, unsigned int Evt_run, std::string dataSetKey, int mctype ){
 ...
 inline float KUCMSTimeCalibration::getCorrectedTime( float time, float amplitude, unsigned int rechitID, unsigned int Evt_run, std::string dataSetKey, int mctype ){
...
}//<<>>float KUCMSTimeCalibration::getCorrectedTime
...
int KUCMSTimeCalibration::getGainId( unsigned int rhID ){
}
*/
...
//----------------------------------------------------------------------------------
 ...
 inline kucms_SigmaFitResult KUCMSTimeCalibration::runTimeFitter( TH2F* hist2D ){
    ...
    ResultsMap["rms"]->Sumw2();
  	...
  	std::map<int,kucms_ProfileTimeFit> profileHists;
	//std::vector<kucms_ProfileTimeFit> profileHists;
  	...
  	for (int ibinX = 1; ibinX < fNBinsX; ibinX++){
		...
		kucms_ProfileTimeFit thisprofile( profilehist );
 ...
 inline TH1F* KUCMSTimeCalibration::gprProfileHist( TH1F* hist ){
...
}//<<>>TH1D* KUCMSTimeCalibration::gprProfileHist( TH1D* hist )
	...
	for( auto& runmaptag : CaliRunMapSet ){
		for( auto& runmap : runmaptag.second ){
			...
			if( runmap.second.has2DResMap && doResFit ){
				...
				kucms_SigmaFitResult results = runTimeFitter( CaliHists[fname].h2f );
 ...
 inline void KUCMSTimeCalibration::doResTimeFit( std::string histName ){
	...
	if( CaliHists.find(histName) != CaliHists.end() ){
		if( CaliHists[histName].h2f != NULL ){
    		kucms_SigmaFitResult results = runTimeFitter( CaliHists[histName].h2f );
