...
cout << "d_100 " << d_100 << " doublecast d_100 " << (double)d_100 << " pi_100 " << pi_100 << " p_dk_h1_100 " << p_dk_h1_100 << " p_dk_tk_100 " << p_dk_tk_100 << " rk_100 " << rk_100 << " rk_d100 " << rk_d100 << " p_dl_100 " << p_dl << " log(p_dl_100) " << log(p_dl) << endl;
	...
	cout << "rk " << rk << " elbo " << elbo << " gamma(n) " << tgamma(n) << " lgamma(n) " << lgamma(n) << " pi " << pi <<  " logpi " << log(pi) << " pi_stable " << pi_stable << " d " << d << " logd " << log(d) << " dr " << r->d << " logdr " << log(r->d) << " dl " << l->d << " logdl " << log(l->d) << " n " << n << " dmin1 " << dmin1 << " d_stable " << d_stable << " p_dk_tkmin1 " << p_dk_tkmin1 << " p_dk_tk_stable " << p_dk_tk_stable << " p_dk_tk_100 " << p_dk_tk_100 << " p_dk_h1_100 " << p_dk_h1_100 << " with # subclusters " << x->model->GetNClusters() << " p(D_l | T_l) " << l->prob_tk << " p(D_r | T_r) }" << r->prob_tk << " log(p_dl) " << log(l->prob_tk) << " log(p_dr) " << log(r->prob_tk) << " n " << n << " npts " << l->points->GetNPoints() + r->points->GetNPoints() << endl;
	//if(_verb > 1){	
