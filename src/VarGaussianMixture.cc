...
//	m_W0.InitRandomSymPosDef();
	...
	if(m_k < 1){
		cout << "Error: need to specify maximum number of clusters (k) expected. Please set this with SetK(k)." << endl;
		return;
	}	
	...
	rs.SetRange(0.,1.);
	for(int k = 0; k < m_k; k++){
		m_betas.push_back(m_beta0);//k*rs.SampleFlat() + 0.1);//rs.SampleFlat());
	}
	...
	for(int k = 0; k < m_k; k++){
		...
		m_means.push_back(mat);//m_mean0);
		mat.clear();
	}
	for(int k = 0; k < m_k; k++){
		mat = Matrix(m_dim, m_dim);
		mat.InitRandomSymPosDef(0.,1.,seed+k);//InitIdentity();
		m_Ws.push_back(mat);//m_W0);
		mat.clear();
	}
	rs.SetRange(m_dim-1,m_dim*1.5);
	//rs.SetRange(m_dim-1,m_dim+2.);
	for(int k = 0; k < m_k; k++)
...
//cout << "CALCULATE POSTERIOR - E STEP" << endl;
	...
	double E_mu_lam, post, norm, expon;
	vector<double> post_norms;
	vector<double> rho; //r_nk = ln(p_nk)/sum_j ln(p_nj)
	double rhoMax = 0; //subtract ln(p_nk)_max from every exponent in denom + norm to avoid numerical blow ups
	...
	//		if(n < 5){ cout << "k: " << k << " n: " << n << " Epi: " << m_Epi[k] << " Elam: " << m_Elam[k] << " E_muLam: " << E_mu_lam << " beta: " << m_betas[k] << " nu: " << m_nus[k]  << " post: " << post << " exp(post): " << exp(post) << " W: " <<  endl; m_Ws[k].Print(); cout << "m: " << endl; m_means[k].Print(); cout << "x: " << endl; m_x.at(n).Print();}	
			//find max ln(p_nk)
			rho.push_back(post);
			//if(fabs(post) > fabs(rhoMax)) rhoMax = post;
			//post = exp(post);
			//norm += post;
			//need to normalize
		...
		//sum norms and subtract rhoMax
		for(int k = 0; k < m_k; k++){
			rho[k] = (rho[k] - rhoMin)/(rhoMax - rhoMin);
			norm += exp(rho[k]);		
	//	if(n < 5) cout << "rhoMax: " << rhoMax << " rhoMin: " << rhoMin << " rho[k] transf: " << rho[k] << " norm: " << norm << endl;
		}
		for(int k = 0; k < m_k; k++)
			m_post.SetEntry(exp(rho[k])/norm, n, k);
		rho.clear();
	}
...
//			if(n < 5) cout << "k: " << k << " n: " << n << " post: " << m_post.at(n,k) << endl;//" norm: " << post_norms[n] << endl;
		}
	}
