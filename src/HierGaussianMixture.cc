...
//k = # of clusters (cols)
//n = # of data pts (rows)
HierGaussianMixture::HierGaussianMixture(){ 
	m_k = 0;
	m_n = 0;
}

HierGaussianMixture::HierGaussianMixture(int k) : BasePDFMixture(k){
	for(int k = 0; k < m_k; k++){
		m_model[k] = new Gaussian();
	}

	m_modelj = new Gaussian();
}

void HierGaussianMixture::InitParameters(unsigned long long seed){
//cout << "HierGaussianMixture::InitParameters alpha0 = " << m_alpha0 << endl;
//	cout << "Gaussian Mixture Model with " << m_k << " clusters for " << m_n << " " << m_dim << "-dimensional points." << endl;
	...
	//seed jet model parameters, mean -> sample mean of MM, cov -> sample cov of MM + S_k
	Matrix mj(m_dim, 1);
	Matrix Sj(m_dim, m_dim);
	for(int k = 0; k < m_k; k++){
...
//E-step
void HierGaussianMixture::CalculatePosterior(){
...
void HierGaussianMixture::UpdateParameters(){
...
double HierGaussianMixture::EvalLogL(){
	double L;
	double sum_k;
	for(int n = 0; n < m_n; n++){
		sum_k = 0;
...
//variational stuff 
void HierGaussianMixture::InitPriorParameters(unsigned long long seed){
//cout << "INIT PRIOR PARAMS" << endl;
	if(m_dim == 0){
		cout << "VarHierGaussianMixture Initialize - Error: data has not been set." << endl;
		return;
	}
...
void HierGaussianMixture::CalculateExpectations(){
//	cout << "CALC EXPECTATIONS" << endl;
	...
	m_Elamj = digam + dim*log(2) + log(scalemat.det());
...
void HierGaussianMixture::CalculateVariationalPosterior(){
...
void HierGaussianMixture::CalculateRStatistics(){
...
//even if the prior on the jet model has dim < m_dim, the model can still be m_dim (just no prior on m_dim - dim = d)
...
//	cout << "Calculate JStats - end" << endl;
}
...
//M-step
void HierGaussianMixture::UpdatePriorParameters(){
//cout << "UPDATE PARAMETERS - M STEP" << endl;
	...
	//if mu.dim != prior.dim
	//assume mu.dim >= prior dim and assume dim matching in order
	if(m_modelj->Dim() > m_modelj->GetPrior()->Dim()){
		int dim = m_modelj->GetPrior()->Dim();
		Matrix newmu(dim,1);
		Matrix newcov(dim,dim);
		for(int d = 0; d < dim; d++){
			newmu.SetEntry(mu.at(d,0),d,0);
			for(int dd = 0; dd < dim; dd++){
				newcov.SetEntry(cov.at(d,dd),d,dd);
			}
		}
		mu = newmu;
		cov = newcov;
	...
	//caluclated for W inverse
	//beta0*N_k/(beta0 + N_k)*(bar{x}_k - m0)(bar{x}_k - m0)T
	Matrix new_scalemat = Matrix(dim, dim);
	//bar{x}_k - m0
	Matrix x_min_mean = Matrix(dim, 1);
	x_min_mean.minus(mu, m_meanj0);
	//cout << "xbar" << endl;
	//mu.Print();
	Matrix x_min_meanT = Matrix(1, dim);
	x_min_meanT.transpose(x_min_mean);
	//(bar{x}_k - m0)(bar{x}_k - m0)T
	new_scalemat.mult(x_min_mean, x_min_meanT);
	//cout << "xxT" << endl;
...
//	cout << "E_q_pi: " << E_q_pi << endl;
	...
	Matrix mj_min_m0 = Matrix(dim,1);
	mj_min_m0.minus(mean,m_meanj0);		
	Matrix mj_min_m0T = Matrix(1, dim);
	mj_min_m0T.transpose(mj_min_m0);
	//(m_j - m_j0)T*W_j0*(m_j - m_j0)
	Matrix mT_x_W = Matrix(1,dim);

	mT_x_W.mult(mj_min_m0T,scalemat);
	Matrix full = Matrix(1,1);
	full.mult(mT_x_W,mj_min_m0);
	
	half_sum += (dim*log(m_betaj0/(2*acos(-1))) + m_Elamj - dim*m_betaj0/scale - m_betaj0*nu*full.at(0,0));
	lam_sum += m_Elamj;

	Matrix tr = Matrix(dim,dim);
	...
	//cout << "for jets - posterior (entropy)" << endl;
	//for jet posterior	
	//E[ln(q(mu, lam))]
	E_q_muLamj = 0;
