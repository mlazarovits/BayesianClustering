...
void Profile2DHist(TH2D* inhist, TH1D* sig_outhist, TH1D* mean_outhist, vector<TH1D*>& profs){
	...
	for(int i = 1; i < nbins+1; i++){
		...
		phist->GetXaxis()->SetTitle(inhist->GetYaxis()->GetTitle());
		//cout << "phist " << phist->GetName() << "  title " << phist->GetTitle() << endl;
...
void Profile2DHists(TFile* f){
	...
	bool print = true;//(filename.find("GMSB") != string::npos);
	...
	while((key = (TKey*)iter())){
		if(key->GetClassName() == tdir){
			...
			dir->cd();
			if(dirname.find("diffDelta") == string::npos){
				continue;
			}
			sig_dirname = dirname;
			...
			//for(auto h : hists) cout << " hist " << h->GetName() << endl;
