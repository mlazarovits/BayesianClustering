...
int EventWeightCalc(string selection = ""){
	...
	vector<string> files;
	files.push_back("GJets_R17"+selection+"_v24_GJets_HT-40To100_AODSIM_RunIIFall17DRPremix");
	...
	for(int f = 0; f < files.size(); f++){
		...
		TFile* file = TFile::Open(("root://cmseos.fnal.gov//store/user/lpcsusylep/malazaro/KUCMSNtuples/"+files[f]+".root").c_str());
		JetProducer* jet_prod = new JetProducer(file);
		TFile* file2 = TFile::Open(("root://cmseos.fnal.gov//store/user/lpcsusylep/malazaro/KUCMSNtuples/"+files[f]+".root").c_str());
