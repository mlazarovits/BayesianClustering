...
void AnyStackHists(string file, string proc, string method, vector<string> matches, string oname, vector<string> types = {}, vector<string> legentries = {}){
				...
				if(types.size() == 0){
					...
					*/
					for(auto h : hists){
						...
						string histname = h->GetName();
						if(histname.find("rStat") != string::npos) continue;	
						if(title.find(matches[m]) == string::npos) h->SetTitle((matches[m]).c_str());
						histstot.push_back(h);
					}
					...
					for(int t = 0; t < types.size(); t++){
						...
						*/
						for(auto h : hists){
							...
							string title = h->GetTitle();
							string histname = h->GetName();
					cout << "xaxis title " << h->GetXaxis()->GetTitle() << endl;
							if(histname.find("rStat") != string::npos) continue;	
							if(title.find(matches[m]) == string::npos) h->SetTitle((matches[m]).c_str());
							histstot.push_back(h);
						}
	...
	if(name.find("sigma") != string::npos || name.find("mean") != string::npos){
		...
		if(string(histstot[0]->GetXaxis()->GetTitle()).empty())
			xlab = name;
		else
			xlab = histstot[0]->GetXaxis()->GetTitle();
		ylab = "a.u."; 
...
void HistFormatSim(string file, string proc = ""){
	...
	vector<string> procs = {"singleW","Wgluon","QCD"};
	//decay types
	vector<string> types = {"b_","qg","lep"};
	vector<string> typesFullDecay = {"fullHad","semiLep","fullLep"};
	vector<string> jettypes_genBHC = {"genAK4","genAK8","genAK15","BHC"};
	...
	if(proc == ""){
		...
		ProcStackHists(file, procs, "BHC", oname, "subclusterEtaTimeCov");
		ProcStackHists(file, procs, "BHC", oname, "subclusterPhiTimeCov");
	...
	MethodStackHists(file, proc, jettypes_recoBHC, oname, "Jet_nSubclustersJet");
	MethodStackHists(file, proc, jettypes_recoAK4BHC, oname, "Jet_nSubclustersJet",pttypes);
	MethodStackHists(file, proc, jettypes_recoAK8BHC, oname, "Jet_nSubclustersJet",pttypes);
	MethodStackHists(file, proc, jettypes_recoAK15BHC, oname, "Jet_nSubclustersJet",pttypes);
	...
	Hist2D(file, proc, "BHC", oname, "jetMass_jetPt");
	Hist2D(file, proc, "recoAK4", oname, "jetMass_jetPt");
